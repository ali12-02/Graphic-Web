...
import { X, FileText, Image, CheckCircle, ArrowRight, ArrowLeft, Loader2, Crop } from 'lucide-react';
      ...
      <div className="w-full max-w-5xl bg-[#0f0f0f] border border-white/10 rounded-3xl shadow-2xl overflow-hidden relative">
        ...
        <div className="flex items-center justify-between px-8 pt-8 pb-4 border-b border-white/5">
          <div className="flex gap-3">
            <button 
              onClick={() => goToStep(1)}
              className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors cursor-pointer hover:opacity-80 ${currentStep >= 1 ? 'bg-purple-600 text-white' : 'bg-white/10 text-gray-400'}`}
            >
              1. Cover
            </button>
            <button 
              onClick={() => goToStep(2)}
              className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors cursor-pointer hover:opacity-80 ${currentStep >= 2 ? 'bg-purple-600 text-white' : 'bg-white/10 text-gray-400'}`}
            >
              2. Details
            </button>
            <button 
              onClick={() => goToStep(3)}
              className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors cursor-pointer hover:opacity-80 ${currentStep === 3 ? 'bg-purple-600 text-white' : 'bg-white/10 text-gray-400'}`}
            >
              3. Publish
            </button>
          </div>
          <button onClick={() => navigate('/dashboard/projects')} className="text-gray-400 hover:text-white transition hover:bg-white/10 p-2 rounded-full">
            <X size={22} />
          </button>
        </div>
                ...
                {!imagePreview ? (
                  <div className="border-2 border-dashed border-gray-700/50 hover:border-purple-500/50 rounded-xl p-10 text-center transition-all duration-300 cursor-pointer bg-[#161616] h-80 flex flex-col items-center justify-center group">
                    ...
                    <label htmlFor="image-upload" className="cursor-pointer flex flex-col items-center w-full h-full justify-center">
                      <div className="w-16 h-16 rounded-full bg-purple-500/10 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform group-hover:bg-purple-500/20">
                  ...
                  // {/* 🟢 CROPPER MODE - PERFECT ALIGNMENT */}
<div className="flex flex-col gap-4 h-auto">
  <div className="relative h-80 rounded-xl overflow-hidden border border-white/10 bg-black">
    <Cropper
      ...
      crop={crop}
      zoom={zoom}
      aspect={aspectRatio === 'FREE' ? undefined : eval(aspectRatio)} 
    ...
    />
                    ...
                    {/* 🟢 CONTROLS BAR - BUTTONS ON FAR RIGHT (End) */}
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-2 bg-[#0a0a0a] px-4 py-2.5 rounded-lg border border-white/10">
                          ...
                          <select 
                            ...
                            className="bg-[#0a0a0a] text-white text-xs border-none outline-none cursor-pointer [&>option]:bg-[#1f1f1f] [&>option]:text-white"
                          >
                        ...
                        <div className="group cursor-default bg-[#0a0a0a] px-4 py-2.5 rounded-lg border border-white/10 flex items-center gap-2 text-xs whitespace-nowrap">
                          ...
                          <span className="text-gray-200 group-hover:text-purple-400 transition-colors font-medium">
                            {pixelDimensions.width}px <span className="text-gray-600">x</span> {pixelDimensions.height}px
                          </span>
                        </div>
                      </div>

                      {/* 🟢 Right Side: Cancel & Apply (Pushed to the absolute right end) */}
                      <div className="flex items-center gap-3">
                        <button 
                          onClick={cancelCrop} 
                          className="px-6 py-2.5 bg-[#0a0a0a] hover:bg-white/10 border border-white/10 text-white text-xs font-medium rounded-lg transition"
                        >
                          Cancel
                        </button>
                        <button 
                          onClick={applyCrop} 
                          className="px-6 py-2.5 bg-purple-600 hover:bg-purple-700 text-white text-xs font-medium rounded-lg transition shadow-lg shadow-purple-900/30"
                        >
                          Apply
                        </button>
                      </div>
                  ...
                  <div className="relative group/image h-80 rounded-xl overflow-hidden border border-purple-500/20 bg-[#0a0a0a]">
                    ...
                    <button type="button" onClick={removeImage} className="absolute top-3 right-3 p-2 bg-red-500/90 hover:bg-red-600 rounded-full transition-all shadow-lg shadow-red-900/50">
              ...
              <div className="space-y-6 bg-[#161616] p-6 rounded-2xl border border-white/5">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full p-4 bg-[#0a0a0a] border border-white/10 rounded-xl text-white placeholder:text-gray-600 focus:border-purple-500 focus:outline-none transition-all" 
                    required 
                  />
                </div>
                ...
                <div>
                  ...
                  <select 
                    ...
                    className="w-full p-4 bg-[#0a0a0a] border border-white/10 rounded-xl text-white focus:border-purple-500 focus:outline-none transition-all [&>option]:bg-[#1f1f1f] [&>option]:text-white appearance-none" 
                    required
                  >
                ...
                <div>
                  ...
                  <textarea 
                    ...
                    rows="4" 
                    className="w-full p-4 bg-[#0a0a0a] border border-white/10 rounded-xl text-white placeholder:text-gray-600 focus:border-purple-500 focus:outline-none transition-all resize-none" 
                    required 
                  />
                </div>
              </div>
              ...
              <div className="bg-[#161616] p-6 rounded-2xl border border-white/5 space-y-6 lg:col-span-1 h-fit">
                ...
                <div>
                  ...
                  <select 
                    ...
                    className="w-full p-3 bg-[#0a0a0a] border border-white/10 rounded-xl text-white focus:border-purple-500 focus:outline-none transition-all [&>option]:bg-[#1f1f1f] [&>option]:text-white"
                  >
            ...
            <div className="flex flex-col items-center justify-center py-6 text-center space-y-6">
              ...
              <div className="bg-[#161616] p-6 rounded-2xl border border-white/5 w-full max-w-md text-left space-y-3 shadow-sm">
                <div className="flex justify-between border-b border-white/5 pb-2">
                  ...
                  <span className="text-white font-medium truncate max-w-[200px]">{formData.title}</span>
                </div>
                <div className="flex justify-between border-b border-white/5 pb-2">
                ...
                <div className="flex justify-between border-b border-white/5 pb-2">
                  ...
                  <span className="text-purple-400 font-medium">{formData.status}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400 text-sm">Cover Image</span>
                  <span className="text-green-400 text-sm font-medium">✅ Cropped & Uploaded</span>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="flex justify-between mt-10 pt-6 border-t border-white/5">
            <button 
              onClick={currentStep === 1 ? () => navigate('/dashboard/projects') : goToPrevStep}
              className="px-6 py-3 text-gray-400 hover:text-white border border-white/10 rounded-xl hover:bg-white/5 transition"
            >
            ...
            {currentStep < 3 ? (
              <button 
                ...
                className="px-8 py-3 bg-white text-black font-medium rounded-xl hover:bg-gray-200 transition flex items-center gap-2 shadow-lg shadow-white/5"
              >
              ...
              <button
                ...
                disabled={loading}
                className={`px-8 py-3 rounded-xl font-semibold text-white transition-all duration-300 flex items-center justify-center gap-2 min-w-[150px]
                  ${loading ? 'bg-gray-700 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700 shadow-lg shadow-green-900/30'}
                `}
              >
